...
/**
 * This is the model class for table "{{%list_contact_type}}".
 *
 * @property string $id
 ...
 */
class ListContactType extends CommonModel
{
    /**
     * @inheritdoc
     */
    ...
    {
        return $this->hasMany(CounterpartyContactPersOthercontact::className(), ['contact_type' => 'id']);
    }
    ...
    public function generateDefaults($params) {
        ...
        if ($params['operation'] != null)
            $this->copyListContactType($params);
    }

    public function copyListContactType($params) {

        if($params['operation'] == self::OPERATION_COPY) {
            $type = ListContactType::findOne(['id' => $params['id']]);
            if($type) {
